...
def RFM(x_l,x_r,counter, x_prev = 0.0, tol = 0.0001,iterations = 10):
    ...
    """
    x_new = (x_l*f(x_r) - x_r*f(x_l))/(f(x_r) - f(x_l))
    counter += 1
    ...
    if((abs(x_prev - x_new)<tol) or (counter > iterations)):
        ...
        RFM(x_l,x_r,counter,x_new,tol,iterations)
